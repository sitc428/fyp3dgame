...
static const irr::u32			CONVERSATION_X1 = 180;
...
static const irr::u32			GAME_START_TIME = 31;
	...
	driver(GEngine->GetDriver())
{
	...
	startGameText.push_back(L"powerful robots, the Black Robot and the Purple");
	...
	startGameText.push_back(L"villagers living nearby and kept destroying");
	startGameText.push_back(L"other villages.");
	startGameText.push_back(L"                                                ");
	startGameText.push_back(L"You and the Purple Robot, luckily survived in the");
	startGameText.push_back(L"accident.  Now, please fight against the monsters");
				...
				for (int i = 0, j = -1; i < ItemBox.size() ; i++){
					if(ItemBox[i].first->getItemType() == HPITEM || ItemBox[i].first->getItemType() == XITEM){
						...
						if (SubMenuIndex == j && j != -1){
							if(ItemBox[i].first->getItemType() == HPITEM && ItemBox[i].second > 0){
								((HPItem*)ItemBox[i].first)->use();
							}
							else if(ItemBox[i].first->getItemType() == XITEM && ItemBox[i].second > 0){
...
void GameHUD::gameVictory(irr::f32 delta)
{
	...
	else if( timeElapsed < GAME_WIN_SHOW_TIME / 6 )
	{
		...
		WinFont->draw(
			L"You Saved The World!",
			irr::core::rect<irr::s32>(0, 0, scrSize.Width, scrSize.Height),
			irr::video::SColor(255 *( 1 - ((local)/ (GAME_WIN_SHOW_TIME / 12)) ), 255, 255, 255),
			true,
			true
		);
	}
...
void GameHUD::DrawBuyingMenu(SellingMachine* sellingmachine, Player& player){
	...
	driver.draw2DImage(SelectIconTexture, irr::core::position2d<irr::s32>(MENU_ITEM_X1-50, MENU_ITEM_Y1+MENU_ITEM_YOFFSET*int(MenuSelected)), irr::core::rect<irr::s32>(0, 0, CD_WIDTH, CD_HEIGTH), 0, irr::video::SColor(255,255,255,255), true);
	MenuFont->draw(L"Buy", irr::core::rect<irr::s32>(MENU_ITEM_X1, MENU_ITEM_Y1, 0, 0), irr::video::SColor(255,255,255,255), false, false, 0);
	MenuFont->draw(L"Exit", irr::core::rect<irr::s32>(MENU_ITEM_X1, MENU_ITEM_Y1+MENU_ITEM_YOFFSET, 0, 0), irr::video::SColor(255,255,255,255), false, false, 0);
	...
	for (int i = 0; i < ItemBox.size() ; i++){
		//output item name
		outputString = ItemBox[i].first->getItemName();
		MenuFont->draw(outputString.c_str(), irr::core::rect<irr::s32>(MENU_WINDOW_X1, MENU_WINDOW_Y1+MENU_ITEM_YOFFSET*i, 0, 0), irr::video::SColor(255,255,255,255), false, false, 0);
