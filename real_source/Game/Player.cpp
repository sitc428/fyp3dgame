...
,	aimVector( vector3df(0.0f,0.0f,1.0f) )
,	translation( vector3df(0.0f,0.0f,0.0f) ) 
...
void Player::SetTranslation( const vector3df& trans )
{
	translation = trans;
}

// set the rotation vector for player
void Player::SetRotation( const vector3df& rot )
{
...
{
/*	static const s32 MAX_CHEERS = 4;
	static s32 randomCheer = ( rand()%MAX_CHEERS );
